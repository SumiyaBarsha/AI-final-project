...
def evaluate_individual(grid, individual, start, goal):
    current = start
...
def run_genetic_algorithm(grid, start, goal, pop_size=100, max_moves=100, num_generations=100, mutation_rate=0.01):
    population = initialize_population(pop_size, max_moves)
    for generation in range(num_generations):
        fitnesses = [evaluate_individual(grid, individual, start, goal) for individual in population]
    ...
    best_individual = max(population, key=lambda ind: evaluate_individual(grid, ind, start, goal))
    ...
    while running:
        ...
        if ga_running:
            ...
            if current_time - ga_start_time >= 1200:
                ...
                ga_generations += 1
                best_path = run_genetic_algorithm(grid, grid[0][0], goal, pop_size=400, max_moves=500, num_generations=1, mutation_rate=0.01)
                current = grid[0][0]
                for move in best_path:
                    ...
                    if current == goal:
                        break
                fitness = evaluate_individual(grid, best_path, grid[0][0], goal)
